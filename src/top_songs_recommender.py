...
## Api call to get top 20 tracks for user
...
## Creating dataframe for track analysis
column_names = ['danceability', 'energy', 'acousticness', 'instrumentalness', 'liveness', 'valence']
df = pd.DataFrame(columns=column_names)

## Create/add recommendations while adding recommendations to dataframe to create radar chart of track analysis
for idx, item in enumerate(top_tracks['items']):
    artistID = [item['artists'][0]['id']]
    trackID = [item['id']]
...
build_radar_chart.createRadar(df)